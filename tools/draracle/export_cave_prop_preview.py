...
from verify_prop_region_height import selected_height
...
def main():
    ...
    chosen={85:195,88:192,47:469,49:473,4:273,5:274,6:276,7:275,56:295,57:296,60:295,61:296,62:296,64:295,8:300,9:299,10:300,11:299,12:300,38:301,39:301,40:301,41:301,42:298,43:297,44:298,45:297,13:302,14:302,15:302,16:302,17:417,18:417,19:417,20:417,21:417,22:278,23:278,24:278,28:306,46:417};states={};state=0;frame=0
    for t in range(count):
        ...
        for selector in range(n):
            ...
            if t in chosen:
                ...
                assert n==1 and c==1 and template[50]==(1 if t in [4,5,6,7,85,88] else 2 if t in [28,56,57,60,61,62,64] else 0) and struct.unpack_from('<h',f)[0]==chosen[t]
    ...
    for k in range(u32(geo,0x60)):
        ...
        region=struct.unpack_from('<H',d,10)[0]
        bounds=dict(states[t])
        floor,ceil=struct.unpack('<hh',struct.pack('<HH',regions[region][10],regions[region][11]))
        bounds['top']=selected_height(bounds['height_flags'],True,floor,ceil,bounds['state_height'])-bounds['top_trim']
        assert bounds['top']>bounds['bottom'] and bounds['right']>bounds['left']
        props.append(dict(record=k,template=t,region=region,position_native=[x,z,-y],**bounds))
    ...
    report=dict(props=props,count=len(props),scope='Original static placement/state dimensions and frame trims. Source resource routing flags0/1/2; flag1 gameplay meaning unresolved in static preview; flag2 region-height selection verified separately. Fixed-Y billboard, first frame, alpha and frame-flag orientation are preview assumptions; no spawn/interaction/collision parity. Original anchors preserved; no floor snapping.')
