...
def main():
 ...
 for group in groups:
  for row in json.loads(next(a.geometry_evidence.glob('*/'+group+'.json')).read_text())['results']:
   ...
   if not mat:deferred['unsupported material']+=1;continue
   pts=[[v/65536 for v in p] for p in row['points_fixed']]
   sloped=pts[0][1]!=pts[1][1] or pts[2][1]!=pts[3][1]
   if pts[0][1]<pts[3][1] or pts[1][1]<pts[2][1]:deferred['crossing vertical boundaries']+=1;continue
   length=math.dist(pts[0][::2],pts[1][::2]);height=max(pts[0][1],pts[1][1])-min(pts[2][1],pts[3][1])
   if length==0 or height<=0:deferred['degenerate span']+=1;continue
   mode=(d[6]>>5)&3;scale=[2.,1.,.5,.25][d[7]&3];anchor,other=[(0,1),(1,0),(3,2),(2,3)][mode];start,end=list(pts[anchor]),pts[other];start[1]=max(pts[0][1],pts[1][1]) if mode<2 else min(pts[2][1],pts[3][1]);dx,dz=(end[0]-start[0])/length,(end[2]-start[2])/length;vertical=-1 if mode<2 else 1
   ...
   result=dict(record=k,region=row['region'],descriptor=mat['descriptor'],variant_count=mat["variant_count"],preview_layout=im.get("preview_layout","unverified"),sloped=sloped,orientation=mode,addressing=addressing,texels_per_unit=[uscale,vscale],points_godot=[[p[0],p[1],-p[2]] for p in pts],uv_unwrapped=[[(max(u,0)+uoffset)/w,(max(v,0)+voffset)/h] for u,v in uv],dimensions=[w,h],offset_bytes=list(d[2:4]))
 ...
 report=dict(exported=len(results),verified_geometry_records=len(seen),geometry_unresolved=3052-len(seen),deferred=dict(deferred),walls=results,scope='Diagnostic inferred world UVs, limited to non-crossing material spans; variants require manual review with three addressing modes; sloped UV anchors use upper/lower envelope. Unwrapped normalized UVs preserve repeats. No collision, transparency, camera or live rendering parity claim; generated files only.')
