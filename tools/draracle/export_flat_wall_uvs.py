...
def main():
 ...
 for group in groups:
  for row in json.loads(next(a.geometry_evidence.glob('*/'+group+'.json')).read_text())['results']:
   ...
   result=dict(record=k,region=row['region'],descriptor=mat['descriptor'],sloped=sloped,orientation=mode,addressing=addressing,texels_per_unit=[uscale,vscale],points_godot=[[p[0],p[1],-p[2]] for p in pts],uv_unwrapped=[[(max(u,0)+uoffset)/w,(max(v,0)+voffset)/h] for u,v in uv],dimensions=[w,h],offset_bytes=list(d[2:4]))
 ...
 report=dict(exported=len(results),verified_geometry_records=len(seen),geometry_unresolved=3052-len(seen),deferred=dict(deferred),walls=results,scope='Diagnostic inferred world UVs, limited to non-crossing static-material spans with three addressing modes; sloped UV anchors use upper/lower envelope. Unwrapped normalized UVs preserve repeats. No collision, transparency, camera or live rendering parity claim; generated files only.')
