...
def main():
 ...
 for group in groups:
  for row in json.loads(next(a.geometry_evidence.glob('*/'+group+'.json')).read_text())['results']:
   ...
   if not d[6]&16:deferred['other addressing mode']+=1;continue
   ...
   uv=[[((pt[0]-start[0])*dx+(pt[2]-start[2])*dz)*scale,(pt[1]-start[1])*vertical*scale] for pt in pts]
   ...
   result=dict(record=k,region=row['region'],descriptor=mat['descriptor'],orientation=mode,texels_per_unit=scale,points_godot=[[p[0],p[1],-p[2]] for p in pts],uv_unwrapped=[[(max(u,0)+d[2])/w,(max(v,0)+d[3])/h] for u,v in uv],dimensions=[w,h],offset_bytes=list(d[2:4]))
   ...
   for y in range(ph):
    for x in range(pw):
     ...
     u,v=address_uv(round(max(values[0],0)*65536),round(max(values[1],0)*65536),d[2],d[3],0,w,h,8);px[x,y]=image.getpixel((u>>16,v>>16))
   filename=f'wall_{k}.png';preview.save(a.out/filename);cards.append(f'<article><h2>Wall {k} · material {mat["descriptor"]}</h2><p>Orientation {mode}, {scale:g} texels/unit</p><img src="{filename}"></article>')
 report=dict(exported=len(results),verified_geometry_records=len(seen),geometry_unresolved=3052-len(seen),deferred=dict(deferred),walls=results,scope='Diagnostic inferred world UVs, limited to rectangular repeating static-material spans. Unwrapped normalized UVs preserve repeats. No collision, transparency, camera or live rendering parity claim; generated files only.')
